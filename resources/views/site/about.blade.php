...
<x-layout title="{{ $title }}" description="{{ __('The story behind the loudest pet treats around.') }}">
        ...
        <div class="mx-auto max-w-7xl px-4 md:px-6 grid lg:grid-cols-2 gap-12 items-center">
            <div class="relative order-2 lg:order-1">
                ...
                <div class="relative aspect-square border-2 border-ink overflow-hidden">
                    <img src="{{ asset('storage/our-history.png') }}" alt="{{ __('Our history') }}" class="h-full w-full object-cover">
                </div>
            </div>
            <div class="order-1 lg:order-2">
                <div class="text-fire text-sm font-bold uppercase tracking-[0.3em]">{{ __('What we do') }}</div>
                <h2 class="mt-3 font-display text-4xl md:text-5xl font-black uppercase leading-tight">
                    {{ __('Just carefully crafted treats you can trust.') }}
                </h2>
                <p class="mt-5 font-editorial italic text-xl md:text-2xl text-ink/80 leading-relaxed">
                    {{ __('We believe dogs are more than pets—they are family. And just like every member of the family, they deserve the very best.') }}
                </p>
                <p class="mt-4 text-ink/75 leading-relaxed">
                    {{ __('That belief is what inspired Delitails.') }}
                </p>
                <p class="mt-4 text-ink/75 leading-relaxed">
    ...
    <x-site.torn-section bg="ink" :top="true">
        <div class="mx-auto max-w-7xl px-4 md:px-6 grid lg:grid-cols-2 gap-12 items-center">
            <div>
                <div class="text-fire-light text-sm font-bold uppercase tracking-[0.3em]">{{ __('Our philosophy') }}</div>
                <h2 class="mt-3 font-display text-4xl md:text-5xl font-black uppercase leading-tight text-bone">
                    {{ __('See the difference in every wag.') }}
                </h2>
                <p class="mt-5 font-editorial italic text-xl md:text-2xl text-bone/80 leading-relaxed">
                    {{ __('Our philosophy is simple: better treats lead to happier, healthier dogs—and you can see the difference in every wag.') }}
                </p>
                <p class="mt-4 text-bone/70 leading-relaxed">
                    {{ __('We are not just another brand—we are a brand created by a producer. Every Delitails product is crafted by us, giving us full control over quality, consistency, and sourcing. This means complete transparency in everything we offer, from the first ingredient to the final treat.') }}
                </p>
            </div>
            <div class="relative">
                ...
                <div class="relative aspect-square border-2 border-bone overflow-hidden">
                    <img src="{{ asset('storage/our-story-feat.jpg') }}" alt="{{ __('Our philosophy') }}" class="h-full w-full object-cover">
                </div>
            </div>
        </div>
    </x-site.torn-section>
    ...
    <x-site.torn-section bg="grass" :top="true">
        <div class="mx-auto max-w-7xl px-4 md:px-6">
            <div class="text-center mb-12">
                <div class="text-ink/70 text-sm font-bold uppercase tracking-[0.3em]">{{ __('What we stand on') }}</div>
                <h2 class="mt-2 font-display text-4xl md:text-6xl font-black uppercase">{{ __('Our four bones') }}</h2>
            </div>
