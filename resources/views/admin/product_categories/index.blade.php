<x-admin.layout title="Product categories">
    <x-slot:actions>
        <a href="{{ route('admin.product-categories.create') }}" class="btn-rough is-fire is-sm">+ New category</a>
    </x-slot:actions>
    ...
    <x-admin.table :headings="['Name','Slug','Products','Active','']">
        @forelse($categories as $cat)
            <tr class="bg-bone hover:bg-fire/5">
                <td class="px-3 py-3"><a class="font-semibold hover:text-fire" href="{{ route('admin.product-categories.edit', $cat) }}">{{ $cat->name }}</a></td>
                ...
                <td class="px-3 py-3 text-right">
                    <div class="flex justify-end gap-2">
                        <a href="{{ route('admin.product-categories.edit', $cat) }}" class="btn-rough is-bone is-sm">Edit</a>
                        <x-admin.confirm-delete :action="route('admin.product-categories.destroy', $cat)"/>
                    </div>
                </td>
            </tr>
        @empty
