...
<div class="grid lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 brush-card p-5 space-y-5" x-data="{ lang: 'en' }" @invalid.capture="lang = 'en'">
        ...
        <div x-show="lang==='el'" x-cloak class="space-y-5">
            <x-admin.form-input name="el[question]" label="Question (Ελληνικά)" :value="$faq->translation('question')"/>
            <x-admin.textarea name="el[answer]" label="Answer (Ελληνικά)" :value="$faq->translation('answer')" rows="8"/>

            <div class="border-t-2 border-dashed border-ink/30 pt-4">
                <h3 class="font-display text-lg font-extrabold uppercase mb-2">SEO</h3>
                <x-admin.form-input name="el[seo_title]" label="SEO title (Ελληνικά)" :value="$faq->translation('seo_title')"/>
                <x-admin.textarea class="mt-3" name="el[seo_description]" label="SEO description (Ελληνικά)" :value="$faq->translation('seo_description')" rows="3"/>
            </div>
        </div>
    </div>
    <div class="space-y-5">
